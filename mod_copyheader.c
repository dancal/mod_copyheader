...
static const char *set_copyheaders(cmd_parms *cmd, void *in_dir_config, const char *header)
{
    copyheader_dir_config *dir_config = in_dir_config;
...
{
    AP_INIT_FLAG("CopyHeaderActive", set_copyheaderactive, NULL, DIR_CMD_PERMS, "Limited to 'on' or 'off'"),
    AP_INIT_TAKE1("CopyHeader", set_copyheaders, NULL, DIR_CMD_PERMS, "a header name to copy to note"), 
	{NULL}
};
...
static apr_status_t copyheader_filter(ap_filter_t *f, apr_bucket_brigade *b)
{
    request_rec *r;
    ...
    r = f->r;
    conf = (copyheader_dir_config *) ap_get_module_config(r->per_dir_config, &copyheader_module);
...
{
    ap_register_output_filter("MOD_COPYHEADER", copyheader_filter, NULL, AP_FTYPE_CONTENT_SET-2);
